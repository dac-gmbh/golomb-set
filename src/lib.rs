//! A Golomb Coded Set is a probabilistic data structure which is typically smaller than a bloom
//! filter but slower to query. A sorted list of differences between samples of a random
//! distribution will roughly have a geometric distribution, which makes for an optimal prefix for
//! Golomb coding. Since a good hash will be randomly distributed, this encoding makes for
//! efficient storage of hashed values.
//!
...
//! ## Usage and Behaviour
//!
//! There are 3 main parameters to select when creating a Golomb Coded Set: the hasher to use, `N`
//! and `P`. `N` should be the maximum number of elements that will be inserted into the set, and
...
//!
//! When selecting a hash it is important to keep in mind it does not have to be cryptographically
//! secure, only evenly distributed. The length must also be greater than `N` * 2 ^ `P`, but this
//! is an easy requirement to meet (even CRC32 would be suitable for a million elements and a false
//! positive probability of 0.001%).
//!
//! ## Example
//!
//! ```rust
